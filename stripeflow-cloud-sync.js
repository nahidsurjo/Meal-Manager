 ...
 * Runs after the existing StripeFlow app and mirrors its state to Supabase.
 * The actual app can keep its local state logic; this layer makes the state
 * persistent across refreshes, browsers and devices.
 */
(function () {
  ...
  async function config() {
    ...
    if (!r.ok) throw new Error('StripeFlow config endpoint failed');
    return r.json();
  }
  ...
  function loginOverlay() {
    const old = document.getElementById('stripeflow-auth');
    if (old) return old;
    const wrap = document.createElement('div');
    wrap.id = 'stripeflow-auth';
    wrap.innerHTML = `
      <div style="position:fixed;inset:0;background:#f7f8fc;display:grid;place-items:center;z-index:99999;font-family:Inter,system-ui,sans-serif">
  ...
  async function signIn() {
    const box = loginOverlay();
    const form = box.querySelector('#stripeflow-login');
    const error = box.querySelector('#sf-login-error');
  ...
  async function pull() {
    ...
    if (data && data.state) {
      ...
      const state = readLocal() || {
        payments: [], withdrawals: [], commissions: [], shorikulPayments: [],
        commissionRate: 4, shorikulRate: 111
      };
      await push(state);
    }
  }
  ...
  function installSaveHook() {
    ...
    localStorage.setItem = function (key, value) {
      ...
      clearTimeout(timer);
      timer = setTimeout(() => {
        try { push(JSON.parse(value)); } catch (_) {}
      }, 250);
    };
  }
  ...
  async function start() {
    try {
      ...
      if (!cfg.url || !cfg.key) throw new Error('Supabase environment variables are not configured in Cloudflare.');
